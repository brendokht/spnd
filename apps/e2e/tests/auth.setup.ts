...
import { OAUTH_USER_STORAGE_STATE, STORAGE_STATE } from "../playwright.config";

const TEST_EMAIL = process.env.TEST_USER_EMAIL ?? "test@example.com";
const OAUTH_EMAIL = oauthUser.email;
const MAILPIT_URL = "http://localhost:54324";

setup("authenticate email user", async ({ page, request }) => {
...
setup("authenticate oauth user", async ({ page, request }) => {
  // 1. Trigger OTP via UI (this is the OTP we need to intercept)
  await page.goto("/login");
  await page.getByLabel(/email/i).fill(OAUTH_EMAIL);
  await page.getByRole("button", { name: /send magic link/i }).click();
  ...
  const latest = mailData.messages
    .filter((m: { To: { Address: string }[] }) =>
      m.To.some((t) => t.Address === OAUTH_EMAIL),
    )
    .sort(
      (a: { Created: string }, b: { Created: string }) =>
        new Date(b.Created).getTime() - new Date(a.Created).getTime(),
  ...
  // 4. Save authenticated session to disk for reuse
  await page.context().storageState({ path: OAUTH_USER_STORAGE_STATE });
});
